...
export default function Button({ variant = 'primary', size = 'medium', animatePress = false, type, children, disabled, processing, processingLabel, className, ...props }: ButtonProps) {
    ...
    const variants = {
        ...
        outline: 'border border-gray-200 hover:bg-gray-200 bg-white',
    ...
    return (
        <button type={type} disabled={disabled || processing} className={clsx(`px-2.5 py-1.5 rounded-lg font-medium transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-1 ${className} ${variants[variant]} ${sizes[size]} `, {
