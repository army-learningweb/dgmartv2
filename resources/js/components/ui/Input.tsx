...
const Input = forwardRef<HTMLInputElement,InputProps>(({ type, name, label, error, ...props }, ref) => {
    return(
        <>
        <label htmlFor={name} className="font-medium text-gray-800 text-[14px]">{label}</label>
            {/* field */}
            <input type={type} name={name} id={name} ref={ref}
                className={clsx("mt-1 border border-gray-200 w-full px-2 py-1.5 rounded-lg focus:outline-0 transition-colors duration-150", {
